...
https://github.com/openeventdata/mordecai/ for instructions on updating.""".format(es_date, mod_date))
    ...
    def _feature_country_mentions(self, doc):# done
        """
    ...
    def _feature_most_common(self, results): # done
        """
    ...
    def _feature_most_alternative(self, results, full_results=False): # done
        """
    ...
    def _feature_most_population(self, results): # done
        """
    ...
    def _feature_word_embedding(self, text): #done
        """
    ...
    def _feature_first_back(self, results): # done
        """
    ...
    def query_geonames(self, placename): # done
        """
    ...
    @lru_cache(maxsize=250)
    def query_geonames_country(self, placename, country): # done
        """
    ...
    def _feature_location_type_mention(self, ent):
        ...
        interest_words = ent.split(" ")#.doc[ent.end - 1 : ent.end + 1]  # last word or next word following

        for word in interest_words:
            if ent in self._just_cts.keys():
                feature_class = "A"
                feature_code = "PCLI"
            elif word.lower() in P_list:
                feature_class = "P"
                feature_code = ""
            elif word.lower() in ADM1_list:
                feature_class = "A"
                feature_code = "ADM1"
            elif word.lower() in ADM2_list:
                feature_class = "A"
                feature_code = "ADM2"
            elif word.lower() in TERRAIN_list:
                feature_class = "T"
                feature_code = ""
            elif word.lower() in AIRPORT_list:
                feature_class = "S"
                feature_code = "AIRP"
            elif word.lower() in A_other:
                feature_class = "A"
                feature_code = ""
    ...
    def make_country_features(self, doc, require_maj=False):
        ...
        for ent in doc:
            ...
            if ent.strip() in self._skip_list:
                continue
            ents.append(ent)
        if not ents:
            return []
        ...
        if self.threads:
            ...
            for ent in ents:
                try:
                    result = self.query_geonames(ent)
                except ConnectionTimeout:
                    result = ""
        ...
        for n, ent in enumerate(ents):
            ...
            ent_label = ent  # destroyed by trimming
    ...
    def format_geonames(self, entry, searchterm=None):
        ...
        """
        try:
            lat, lon = entry['coordinates']["lat"],entry['coordinates']["lon"]
